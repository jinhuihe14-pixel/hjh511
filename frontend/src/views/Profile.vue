...
const stats = ref({ orderCount: 0, revenue: 0 })
...
const loadData = async () => {
  ...
  const monthOrders = orders.filter(o => new Date(o.createdAt) >= new Date(monthStart))
  
  stats.value = {
    orderCount: monthOrders.length,
    revenue: monthOrders.reduce((sum, o) => sum + (o.actualAmount || 0), 0)
  }
